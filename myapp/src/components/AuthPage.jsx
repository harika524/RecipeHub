...
// AuthPage component handles the Home, Sign Up, and Sign In views.
// It is now a self-contained page with its own header and footer.
const AuthPage = () => {
  const navigate = useNavigate(); // Use useNavigate directly within this component
  ...
  const handleSignUpSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEmail('');
        setPassword('');
        setCurrentView('signin'); // Automatically switch to sign-in view after successful signup
      } else {
  ...
  const handleSignInSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUsername('');
        setPassword('');
        navigate('/welcome'); // Redirect to the dedicated WelcomePage.jsx
      } else {
        ...
        return (
          <div style={{ ...styles.formContainer, animation: 'fadeInUp 0.8s ease-out' }}>
            ...
            <p style={styles.formSwitchText}>
              Already have an account? <span style={styles.formSwitchLink} onClick={() => setCurrentView('signin')}>Sign In</span>
            </p>
          </div>
        );
      case 'signin':
        return (
          <div style={{ ...styles.formContainer, animation: 'fadeInUp 0.8s ease-out' }}>
            ...
            {message && <p style={styles.message}>{message}</p>}
            <p style={styles.formSwitchText}>
              Don't have an account? <span style={styles.formSwitchLink} onClick={() => setCurrentView('signup')}>Sign Up</span>
            </p>
          </div>
        );
      default:
  ...
  return (
    <div style={styles.wrapper}> {/* Re-added wrapper */}
      {/* Global CSS embedded (for keyframes) */}
      <style>
        {`
        body, html, #root {
          margin: 0;
      ...
      <div style={styles.header}>
        ...
        <div style={styles.navLinks}>
          <span
            ...
            onClick={() => setCurrentView('home')} // Internal view change
          >
            Home
          </span>
          <span
            ...
            onMouseLeave={() => setHoveredNav(null)}
            onClick={() => setCurrentView('signup')} // Internal view change
          >
            Sign Up
          </span>
          <span
            ...
            onMouseLeave={() => setHoveredNav(null)}
            onClick={() => setCurrentView('signin')} // Internal view change
          >
            Sign In
          </span>
          <span
            ...
            onMouseLeave={() => setHoveredNav(null)}
            onClick={() => navigate('/contact')} // External navigation
          >
            Contact
          </span>
          <span
            ...
            onMouseLeave={() => setHoveredNav(null)}
            onClick={() => navigate('/welcome')} // External navigation
          >
            Welcome
          </span>
        </div>
...
// Styles object, adapted from WelcomePage.jsx and ContactPage.jsx
const styles = {
  wrapper: {
    display: 'flex',
